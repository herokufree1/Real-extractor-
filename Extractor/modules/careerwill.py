...
async def career_will(app: Client, message: Message):
    try:
        welcome_msg =(
         "<blockquote>**Send ID & Password in this manner otherwise bot will not respond.\n\nSend like this:-  ID*Password\n\n OR Send Your Token**</blockquote>"
        )
        ...
        if "*" in raw_text:
            ...
            success_msg = (
                "<blockquote>**Login Successful**\n\n"
                f"<b>Id Password:</b> <code>{email}*{password}</code>"
            )
        ...
        msg = "<b>Available Batch</b>\n\n"
        for b in batches:
            msg += f"<code>{b['id']}</code> - <b>{b['batchName']}</b>\n"

        await message.reply_text(msg)
        input2 = await app.ask(message.chat.id, "<b>Send the Batch ID :</b>")
        ...
        prog = await message.reply(
            "<b>Processing Course --𝔸ℕ𝕁𝔸ℕ ℙ𝔼ℝ𝕊𝕆ℕ™</b>\n\n"
            "─Status: Extracting \n"
            "─ Please wait--"
        )
        ...
        error_msg = (
            ...
            f"Error details: <code>{str(e)}</code>\n\n"
            "Please try again or contact Anjanperson"
        )
        await message.reply(error_msg)
